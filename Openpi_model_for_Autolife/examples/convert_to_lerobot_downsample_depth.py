...
Data directory structure expected:
    data_dir/
    ├── session_001/
    │   ├── dataset_session_20260313_174013.jsonl
    │   └── videos/
...
# ─────────────────────────────────────────────
# ADD: Depth camera config
# ─────────────────────────────────────────────
DEPTH_CAMERA = "rgbd_head_depth"   # change if your file has different name
DEPTH_CAMERA_EXT = ".mkv"   
# Your RGB-D camera depth range — adjust to match your camera spec
# RealSense D series: typically 0.3m to 3.0m, raw values in mm (uint16)
DEPTH_SCALE_MM_TO_M = 1000.0       # divide raw uint16 by this → meters
MAX_DEPTH_METERS    = 1.5          # clip anything beyond this (noisy)
...
FPS = 2
DOWNSAMPLE_STRIDE = 15  # 30Hz / 2Hz = every 15th frame
...
def get_frame_from_depth_camera(depth_cap, frame_idx):
    """
    Read depth frame from RealSense Z16 MKV.
    Disable OpenCV's RGB conversion to get raw uint16 mm values directly.
    """
    ...
    # frame is already uint16 mm values — just convert to float32
    return frame.astype(np.float32)


def open_depth_capture(video_path):
    """
    Open depth MKV with RGB conversion disabled
    so OpenCV returns raw uint16 depth values.
    """
    cap = cv2.VideoCapture(str(video_path))
    cap.set(cv2.CAP_PROP_CONVERT_RGB, 0)   # ← critical line
    return cap

def convert_raw_depth_to_grayscale_and_resize(depth_raw, image_size=224):
    """
    Convert raw depth frame from RGB-D camera into format
    QDepth-VLA's VQ-VAE expects.

    Input:  raw float32 array (H, W) — depth in mm from camera
    Output: uint8 numpy array (224, 224, 3) — normalized grayscale, 3-channel
    
    Convention: close = bright (255), far = dark (0)
    This matches Video-Depth-Anything output that VQ-VAE was trained on.
    """
    # Step 1: mm → meters
    depth_m = depth_raw / DEPTH_SCALE_MM_TO_M

    # Step 2: replace invalid/zero depth with max range
    depth_m[depth_m <= 0] = MAX_DEPTH_METERS

    # Step 3: clip to valid range
    depth_m = np.clip(depth_m, 0.0, MAX_DEPTH_METERS)

    # Step 4: normalize to [0, 255] — close=bright, far=dark
    depth_norm = ((MAX_DEPTH_METERS - depth_m) / MAX_DEPTH_METERS * 255.0)
    depth_uint8 = depth_norm.astype(np.uint8)

    # Step 5: resize to 224x224
    depth_resized = cv2.resize(depth_uint8, (image_size, image_size),
                               interpolation=cv2.INTER_LINEAR)

    # Step 6: stack to 3-channel — VQ-VAE expects (H, W, 3)
    depth_3ch = np.stack([depth_resized] * 3, axis=-1)

    # 2. 建立 Mask：找出所有數值為 0（黑色無效區）的像素
    mask = (depth_uint8 == 0).astype(np.uint8) * 255

    # 3. 使用 OpenCV 內建的修補函式 (INPAINT_TELEA 或 INPAINT_NS)
    # 參數 3 是修補半徑，可以根據孔洞大小調整（通常 3~5 即可）
    filled_depth = cv2.inpaint(depth_uint8, mask, inpaintRadius=5, flags=cv2.INPAINT_TELEA)

    # 4. 儲存結果
    cv2.imwrite("filled_frame.png", filled_depth)
...
def load_session(session_dir: Path):
    ...
    video_caps = {}
    for cam_name in ["rgbd_head_color", "hand_right"]:
    ...
    # ADD: open depth video capture too
    depth_video_path = video_dir / f"{DEPTH_CAMERA}{DEPTH_CAMERA_EXT}"
    depth_cap = None
    if depth_video_path.exists():
        depth_cap = open_depth_capture(depth_video_path)
        print(f"  Depth video found: {depth_video_path}")
    else:
        print(f"  WARNING: No depth video found at {depth_video_path}")

    # Return depth_cap alongside video_caps
    return frames, video_caps, depth_cap   # ← add depth_cap to return
...
def convert_sessions_to_lerobot(
    ...
    for session_dir in tqdm(session_dirs, desc="Sessions"):
        ...
        try:
            ...
            frames, video_caps, depth_cap = load_session(session_dir)
        ...
        # ADD after your existing img_check / wrist_check lines
        depth_raw_check = get_frame_from_depth_camera(depth_cap, test_i)
        depth_img_check = convert_raw_depth_to_grayscale_and_resize(depth_raw_check)

        # Save depth frame alongside RGB for visual inspection
        combined_check = np.hstack([
            cv2.cvtColor(img_check,       cv2.COLOR_RGB2BGR),
            cv2.cvtColor(wrist_check,     cv2.COLOR_RGB2BGR),
            depth_img_check,               # already BGR-compatible (grayscale 3ch)
        ])
        # Print depth stats — sanity check values look reasonable
        print(f"Depth raw min/max: {depth_raw_check.min():.0f} / {depth_raw_check.max():.0f} mm")
        print(f"Depth normalized min/max: {depth_img_check.min()} / {depth_img_check.max()}")
        # print(f"Expected: raw ~300-2000mm, normalized 0-255")
        ...
        for idx, i in enumerate(sampled_indices):
            frame      = frames[i]
            next_frame = frames[i + DOWNSAMPLE_STRIDE]
            ...
            depth_raw  = get_frame_from_depth_camera(depth_cap, i)
            depth_img  = convert_raw_depth_to_grayscale_and_resize(depth_raw)
            # ──────────────────────────

            dataset.add_frame({
                "image":       main_img,
                "wrist_image": wrist_img,
                "depth_image": depth_img,    # ← ADD THIS
                "state":       np.array(state,  dtype=np.float32),
                "actions":     np.array(action, dtype=np.float32),
                "task":        task,
            })
