...
def convert_sessions_to_lerobot(
    ...
    """
    Main conversion function. Iterates over task directories (pick_up_cube, place_down_cube)
    and converts all underlying sessions into a single combined LeRobot dataset.
    """
    ...
    # Create one unified LeRobot dataset
    ...
    # Look for the subfolders inside your main data directory
    task_folders = [d for d in data_path.iterdir() if d.is_dir()]
    
    if not task_folders:
        raise ValueError(f"No task subdirectories found in {data_dir}")

    total_frames = 0

    # Loop through each task folder (e.g., pick_up_cube, place_down_cube)
    for task_folder in task_folders:
        # Use the folder's name directly as the language prompt token!
        # This replaces underscores with spaces for natural language (e.g., "pick up cube")
        current_task_prompt = task_folder.name.replace("_", " ")
        print(f"\n==================================================")
        print(f"📂 Scanning Task Folder: {task_folder.name} -> Label: '{current_task_prompt}'")
        print(f"==================================================")

        # Find all session directories inside this specific task folder
        session_dirs = sorted([
            d for d in task_folder.iterdir()
            if d.is_dir() and any(d.glob("joints/*.jsonl")) 
        ])

        print(f"Found {len(session_dirs)} session(s) for task '{current_task_prompt}'")
        ...
        for session_dir in session_dirs:
            ...
            # Check if videos are properly bound
            wrist_cap = video_caps.get(WRIST_CAMERA)
            if wrist_cap is None:
                print(f"    ⚠️ Warning: Missing video for {WRIST_CAMERA} in {session_dir.name}. Skipping.")
                continue
            ...
            # Build list of downsampled indices
            ...
            for idx, i in enumerate(sampled_indices):
                ...
                # Append frame with the dynamically chosen string prompt
                dataset.add_frame({
                    "image":       main_img,
                    "wrist_image": wrist_img,
                    "state":       np.array(state,  dtype=np.float32),
                    "actions":     np.array(action, dtype=np.float32),
                    "task":        current_task_prompt, 
                })

            print(f"    Added {len(sampled_indices)} downsampled frames.")
            dataset.save_episode()
            total_frames += len(sampled_indices)

            # Close video streams for this session
            for cap in video_caps.values():
                if cap is not None:
                    cap.release()

    print(f"\n✅ All tasks converted completely!")
    print(f"Total compiled dataset frames: {total_frames}")
    print(f"Dataset saved locally to: {dataset.root}")

    # Push to Hugging Face Hub
    if push_to_hub:
        print(f"\nPushing combined dataset to Hugging Face Hub as '{repo_id}'...")
        dataset.push_to_hub(
